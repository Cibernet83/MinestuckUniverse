...
public class BadgeUtilSpace extends BadgeHeroAspectUtil
{
	...
	public boolean onBadgeTick(World world, EntityPlayer player, IBadgeEffects badgeEffects, SkillKeyStates.KeyState state, int time)
	{
		...
		if (state == SkillKeyStates.KeyState.RELEASED)
		{
			...
			if (time >= 20)
				badgeEffects.startPowerParticles(BadgeUtilSpace.class, MSUParticles.ParticleType.BURST, EnumAspect.SPACE, 10);
			else badgeEffects.startPowerParticles(BadgeUtilSpace.class, MSUParticles.ParticleType.AURA, EnumAspect.SPACE, 5);
		}
		else badgeEffects.startPowerParticles(BadgeUtilSpace.class, MSUParticles.ParticleType.AURA, EnumAspect.SPACE, (time >= 20 && pos1 != null && pos2 != null) ? 6 : 1);
