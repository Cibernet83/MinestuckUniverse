...
public class MasterBadge extends BadgeLevel
{
	...
	{
		return Math.max(0, Math.min(limit + (player.getCapability(MSUCapabilities.GOD_TIER_DATA, null).isBadgeActive(MSUBadges.BADGE_OVERLORD) ? 20 : 0), (player.getLuck()+1)*baseStat));
	}

	@Override
	@SideOnly(Side.CLIENT)
	public String getDisplayTooltip() {
		return I18n.format(getUnlocalizedName() + ".tooltip", getStatNumber(Minecraft.getMinecraft().player), (MSUBadges.MASTER_BADGE_WISE.getStatNumber(Minecraft.getMinecraft().player)/2));
	}
	...
	public ResourceLocation getTextureLocation()
	{
		return new ResourceLocation(getRegistryName().getResourceDomain(), "textures/gui/badges/"+getRegistryName().getResourcePath() +
																		   (Minecraft.getMinecraft().player.getCapability(MSUCapabilities.GOD_TIER_DATA, null).isBadgeActive(MSUBadges.BADGE_OVERLORD) ? "_overlord" : "") +".png");
	}
}
